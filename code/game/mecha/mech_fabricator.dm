...
/obj/machinery/mecha_part_fabricator/Topic(href, href_list)
	...
	if(href_list["part_set"])
		var/tpart_set = mfilte.getStr("part_set")
		if(tpart_set)
			if(tpart_set=="clear")
				part_set = null
			else
	...
	if(href_list["add_to_queue"])
		var/T = mfilte.getStr("add_to_queue")
		for(var/v in files.known_designs)
			var/datum/design/D = files.known_designs[v]
			if(D.build_type & MECHFAB)
				if(D.id == T)
	...
	if(href_list["remove_from_queue"])
		remove_from_queue(mfilte.getNum("remove_from_queue"))
		return update_queue_on_page()
	if(href_list["partset_to_queue"])
		add_part_set_to_queue(mfilte.get("partset_to_queue"))
		return update_queue_on_page()
	...
	if(href_list["queue_move"] && href_list["index"])
		var/index = mfilte.getNum("index")
		var/new_index = index + mfilte.getNum("queue_move")
