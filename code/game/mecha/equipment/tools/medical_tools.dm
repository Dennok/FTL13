...
/obj/item/mecha_parts/mecha_equipment/medical/sleeper/Topic(href,href_list)
	...
	if(mfilte.get("eject"))
		go_out()
	if(mfilte.get("view_stats"))
		chassis.occupant << browse(get_patient_stats(),"window=msleeper")
		onclose(chassis.occupant, "msleeper")
		return
	if(mfilte.get("inject"))
		inject_reagent(mfilte.getType("inject", /datum/reagent),mfilte.getObj("source"))
	return
...
/obj/item/mecha_parts/mecha_equipment/medical/syringe_gun/Topic(href,href_list)
	..()
	var/datum/topic_input/mfilte = new (href,href_list)
	if(mfilte.get("toggle_mode"))
		mode = !mode
		update_equip_info()
		return
	if(mfilte.get("select_reagents"))
		...
		for(var/i=1 to known_reagents.len)
			if(m>=synth_speed)
				break
			var/reagent = mfilte.get("reagent_[i]")
		...
		return
	if(mfilte.get("show_reagents"))
		chassis.occupant << browse(get_reagents_page(),"window=msyringegun")
	if(mfilte.get("purge_reagent"))
		var/reagent = mfilte.get("purge_reagent")
		if(reagent)
			reagents.del_reagent(reagent)
		return
	if(mfilte.get("purge_all"))
