...
/obj/mecha/Topic(href, href_list)
	...
	var/datum/topic_input/mfilte = new /datum/topic_input(href,href_list)

	if(in_range(src, usr))

		if(href_list["req_access"] && add_req_access)
			output_access_dialog(mfilte.getObj("id_card"),mfilte.getMob("user"))

		if(href_list["maint_access"] && maint_access)
			var/mob/user = mfilte.getMob("user")
			if(user)
				if(state==0)
					state = 1
				...
				output_maintenance_dialog(mfilte.getObj("id_card"),user)
		...
		if(href_list["add_req_access"] && add_req_access && mfilte.getObj("id_card"))
			operation_req_access += mfilte.getNum("add_req_access")
			output_access_dialog(mfilte.getObj("id_card"),mfilte.getMob("user"))

		if(href_list["del_req_access"] && add_req_access && mfilte.getObj("id_card"))
			operation_req_access -= mfilte.getNum("del_req_access")
			output_access_dialog(mfilte.getObj("id_card"),mfilte.getMob("user"))
	...
	if(href_list["select_equip"])
		var/obj/item/mecha_parts/mecha_equipment/equip = mfilte.getObj("select_equip")
	...
	if(href_list["rfreq"])
		var/new_frequency = (radio.frequency + mfilte.getNum("rfreq"))
